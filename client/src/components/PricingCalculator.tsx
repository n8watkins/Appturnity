  ...
  const prefillFromQuiz = (data: any) => {
    ...
    if (data.pageCount) {
      ...
      if (pageRange === "1-5") pageValue = 3;
        ...
        pageValue = 25; // Premium tier (> 20 pages)
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-lg text-slate-300">
            Configure your project, select features, and see your investment instantly. Pay once,
            own forever.
          </p>
              ...
              <AnimatePresence mode="wait">
                {showComparison && !isCalculating && (
                  ...
                  >
                        ...
                        <div className="space-y-2 mb-4 text-sm flex-grow">
                          <div className="flex justify-between">
                            ...
                            <span className="font-semibold text-green-900">
                              ...
                              {(() => {
                                ...
                                return tierBases[
                                  pageTier as keyof typeof tierBases
                                ].toLocaleString();
                              })()}
                            </span>
                          </div>
                          ...
                          {(() => {
                            const advancedFeatures = features.filter(
                              (f) => f.enabled && f.price > 0
                            );
                            const sortedByPrice = [...advancedFeatures].sort(
                              (a, b) => a.price - b.price
                            );

                            const includedFeatures = sortedByPrice.filter((f, index) => {
                              const isIncluded =
                                includedAdvancedFeatures !== Infinity &&
                                index < includedAdvancedFeatures;
                          ...
                          {prefilledFromQuiz && quizDiscount > 0 && (
                            <div className="flex justify-between text-xs bg-yellow-100 p-1.5 rounded -mx-1">
                              <span className="text-yellow-900 font-semibold">
                                🎉 Quiz Discount ({QUIZ_DISCOUNT_PERCENT}%):
                              </span>
                        ...
                        <div className="space-y-2 mb-4 text-sm flex-grow">
                          <div className="flex justify-between">
                            <span className="text-red-800">
                              {pages} {pages === 1 ? "page" : "pages"} × $8/page:
                            </span>
                            <span className="font-semibold text-red-900">${saasPageCost}/mo</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-red-800">
                              {users} {users === 1 ? "user" : "users"} × $12/user:
                            </span>
                    ...
                    >
                      <p className="text-xs font-medium mb-1 uppercase tracking-wide opacity-90">
                        Total Savings Compared to SaaS Platforms
                      </p>
                      ...
                      <div className="space-y-1 opacity-90">
                        <p className="text-sm">
                          Pay once: <strong>${totalPrice.toLocaleString()}</strong> vs SaaS total
                          over 3 years:{" "}
                          <strong>${Math.round(saasThreeYearTotal).toLocaleString()}</strong>
                        </p>
                        <p className="text-xs opacity-75">
                          Save an average of $
                          {Math.round((saasThreeYearTotal - totalPrice) / 36).toLocaleString()}
                          /month by owning your solution
                        </p>
                      </div>
                    </motion.div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6 text-xs text-slate-600 space-y-2">
                      ...
                      <ul className="space-y-1 ml-5 list-disc">
                        <li>
                          <strong>SaaS pricing sources:</strong> Based on published pricing from
                          Webflow ($29-212/mo), Wix ($27-159/mo), Squarespace ($25-65/mo), Shopify
                          ($39-399/mo), and HubSpot CMS ($23-1,200/mo)
                        </li>
                        <li>
                          <strong>What's excluded:</strong> Comparison focuses on platform and
                          feature costs only. Standard operational costs like SSL certificates,
                          domain registration, and hosting are typically included in both solutions
                          and not factored into the savings calculation.
                        </li>
                        <li>Your quote is fixed for 30 days from today</li>
                        <li>Includes {calculateTimeline()} delivery timeline</li>
                        <li>
                          Hosting and SSL certificates included for first year ($200/year
                          thereafter)
                        </li>
                            ...
                            ([category, categoryFeatures]) => {
                              ...
                              return (
                                <div key={category}>
                                  ...
                                  {/* Features Grid */}
                                  <div
                                    className={`grid grid-cols-3 gap-2.5 ${isAlwaysIncluded ? "mb-6" : ""}`}
                                  >
                              ...
                              <div className="space-y-0.5 text-xs text-green-700 ml-2.5">
                                ...
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span>
                                    {includedAdvancedFeatures === Infinity
                                      ? "Unlimited"
                                      : includedAdvancedFeatures}{" "}
                                    advanced{" "}
                                    {includedAdvancedFeatures === 1 ? "feature" : "features"}{" "}
                                    included
                                  </span>
                                </div>
                              </div>
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm font-semibold text-yellow-900">
                                        Quiz Completion Discount ({QUIZ_DISCOUNT_PERCENT}%)
                                      </span>
                                    </div>
